Attribute VB_Name = "ZAPCLAIM_SP01_NP"
Public BDPass As String
Public BDUserName As String



Sub Run_ZAPCLAIM_SPO1_Allowance_NP()
...
"Issue date for the Non-Perodic Paper Claim:     " & ZAPCLAIM_Req_Issue_Date & vbNewLine & vbNewLine & _
"Issue date for the Non-Perodic EFT Claim:       " & ZAPCLAIM_Req_Issue_Date & vbNewLine & vbNewLine & vbNewLine & _
...
F110_Run_Date = wb.Sheets("Macro Input").Range("F110_Run_Date")
  ...
  If BET_RangeNameExists("CB_NP_F110_CB0P") = True Then
        Application.GoTo Reference:="CB_NP_F110_CB0P"
        CB_NP_F110_CB0P = wb.ActiveSheet.Range("CB_NP_F110_CB0P")
End If


If BET_RangeNameExists("CB_NP_F110_CB0T") = True Then
            Application.GoTo Reference:="CB_NP_F110_CB0T"
            CB_NP_F110_CB0T = wb.ActiveSheet.Range("CB_NP_F110_CB0T")
...
'Generate claim and print claim schedule for paper allowance roll payments
 ...
 If CB_NP_F110_CB0P > 0 Then
                ...
                If Printed = "Printed succesfully" Then
                        wb.Sheets("Macro Input").Range("CS_5_PRINT") = "X"
                End If
...
End If
 ...
 'Generate claim and print claim schedule for EFT allowance roll payments
 
 If BET_RangeNameExists("CB_NP_F110_CB0T") = True Then
 If CB_NP_F110_CB0T > 0 Then
                ...
                SAP_Session.FindById("wnd[0]/usr/ctxtP_CSTYP").Text = "CBEFT"
                SAP_Session.FindById("wnd[0]/usr/ctxtSO_LAUFD-LOW").Text = F110_Run_Date
                SAP_Session.FindById("wnd[0]/usr/ctxtP_RQDAT").Text = ZAPCLAIM_Req_Issue_Date
                SAP_Session.FindById("wnd[0]/usr/chkP_UPD").Selected = True
                SAP_Session.FindById("wnd[0]/usr/txtSO_LAUFI-LOW").Text = "CB0T"
                SAP_Session.FindById("wnd[0]/tbar[1]/btn[8]").press
                
                
                EFT_Claim_Number = SAP_Session.FindById("wnd[0]/usr/lbl[15,2]").Text
                wb.Sheets("Macro Input").Range("CS_6") = EFT_Claim_Number
                SAP_Session.FindById("wnd[0]/tbar[0]/btn[15]").press
                
                
                SAP_Session.FindById("wnd[0]/usr/radP_PRI").Select
                SAP_Session.FindById("wnd[0]/usr/chkP_FSIND").Selected = True
                SAP_Session.FindById("wnd[0]/usr/ctxtP_CSNBR").Text = EFT_Claim_Number
                SAP_Session.FindById("wnd[0]/usr/ctxtP_FSHPR").Text = "LOCL"
                SAP_Session.FindById("wnd[0]/usr/ctxtP_FSFRM").Text = "ZFI_CLAIM_FACESHEET"
                SAP_Session.FindById("wnd[0]/usr/ctxtP_FSHPR").SetFocus
                SAP_Session.FindById("wnd[0]/usr/ctxtP_FSHPR").CaretPosition = 4
                ...
                If Printed = "Printed succesfully" Then
                        wb.Sheets("Macro Input").Range("CS_6_PRINT") = "X"
                End If
...
"The macro has finished running ZAPCLAIM for CB Non-Periodic Payments." & vbNewLine & vbNewLine & _
"Please press OK."
