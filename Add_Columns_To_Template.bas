Attribute VB_Name = "Add_Columns_To_Template"
Sub Add_Columns_To_Import_Template_1()
...
'=========Change both Named_Range_2 to other number or toother named range to map other tables to a template============
  If BET_RangeNameExists(Named_Range_1) = True Then
        Application.GoTo Reference:=Named_Range_1
        MapperRange = ThisWorkbook.ActiveSheet.Range(Named_Range_1)
Else
        MsgBox ("ERROR: The cell Named_Range_1 is empty on the 'Macro Input' tab, or the named range referenced there is misspelled or does not exist in this workbook." & vbNewLine & vbNewLine & "Please click OK to end the macro.")
        End
End If
'=====================================================================================================================
                        ...
                        If BET_RangeNameExists(nm) = True Then
                                       ...
                                       '===========Set up the 'rowpaste' variable, the last row where data is pasted in.  If it's not the first row, then add 8 rows for the header, and also add rows_added previously add if it's on a work order other than the first=======
                                        ...
                                        'add rows if out of space on the template
                                                    Do While (rows_to_be_added_to_Template + rows_added + 10) > templaterows
